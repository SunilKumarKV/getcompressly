...
export async function getUsageLimit(user: Express.User | undefined) {
  ...
  return {
    dailyLimit: plan?.dailyLimit ?? (user.plan === PlanType.PRO || user.plan === PlanType.PRO_MONTHLY || user.plan === PlanType.PRO_YEARLY ? env.PRO_DAILY_LIMIT : env.FREE_DAILY_LIMIT),
    maxFileSizeMb: plan?.maxFileSizeMb ?? (user.plan === PlanType.PRO || user.plan === PlanType.PRO_MONTHLY || user.plan === PlanType.PRO_YEARLY ? env.PRO_MAX_FILE_SIZE_MB : env.FREE_MAX_FILE_SIZE_MB),
    batchLimit: plan?.batchLimit ?? (user.plan === PlanType.PRO || user.plan === PlanType.PRO_MONTHLY || user.plan === PlanType.PRO_YEARLY ? 20 : 3),
    plan: user.plan
  };
}
